...
class MappingPage(BasePage):
    ...
    def __init__(self, parent):
        ...
        self._resource_request_in_flight = False
    ...
    def change_window_name(self):
        ...
        self.parent.title(f"{APP_NAME} - Button Mapping")

    def create_widgets(self):
        """Create widgets for the mapping page."""
        # Title frame
        self.add_title("Hardware Mapping")
        ...
        # Three complete rows of four hardware controls (B0-B3, P0-P3, G0-G3).
        for row, prefix in enumerate(("B", "P", "G")):
            for column in range(4):
                self.frames["deck_layout"].grid_columnconfigure(column, weight=1)
                self.create_mapping_button(f"{prefix}{column}", row, column)

        self.labels["deck_label"] = customtkinter.CTkLabel(
            self.frames["deck_layout"],
            text="Hardware layout - 12 controls",
            font=customtkinter.CTkFont(size=12, weight="bold"),
        )
        self.labels["deck_label"].grid(
            row=3, column=0, columnspan=4, padx=5, pady=(10, 5)
        )
        ...
        self.comboboxes["scene_select"] = customtkinter.CTkComboBox(
            scene_tab, values=["Loading scenes..."]
        )

        self.buttons["assign_scene"] = customtkinter.CTkButton(
            scene_tab, text="Assign Scene", command=self.assign_scene
        )
        ...
        self.comboboxes["source_select"] = customtkinter.CTkComboBox(
            volume_tab, values=["Loading sources..."]
        )

        self.buttons["assign_source"] = customtkinter.CTkButton(
            volume_tab, text="Assign Source", command=self.assign_source
        )
        ...
        self.comboboxes["script_select"] = customtkinter.CTkComboBox(
            script_tab, values=["Loading scripts..."]
        )

        self.buttons["assign_script"] = customtkinter.CTkButton(
            script_tab, text="Assign Script", command=self.assign_script
        )
        ...
        self.buttons["save_mappings"] = customtkinter.CTkButton(
            self.frames["bottom"], text="Save Mappings", command=self.save_mappings
        )
    ...
    def button_clicked(self, button_id):
        ...
        if assignment:
            self.labels["current_assignment"].configure(
                text=f"Current Assignment: {assignment}"
            )
        else:
    ...
    def update_button_text(self, button_id):
        ...
        if assignment:
            ...
            self.buttons[button_id].configure(
                text=f"{base_text} {button_id}\n{short_name}"
            )
        else:
    ...
    def on_show(self):
        ...
        # Load external resources without blocking the Tk event loop.
        self.load_obs_resources()
        self.load_scripts()

    def load_obs_resources(self):
        """Fetch scenes and sources on a worker and apply one UI snapshot."""
        if self._resource_request_in_flight:
            return
        if not self.app.obs or not self.app.obs.is_connected():
            self._apply_obs_resources(([], []), connected=False)
            return

        self._resource_request_in_flight = True
        self.comboboxes["scene_select"].configure(values=["Loading scenes..."])
        self.comboboxes["source_select"].configure(values=["Loading sources..."])

        def fetch():
            return (
                self.app.obs.get_scene_list() or [],
                self.app.obs.get_volume_sources() or [],
            )

        self.app.submit_background(
            fetch,
            self._apply_obs_resources,
            self._obs_resources_error,
            description="OBS mapping resources",
        )

    def _apply_obs_resources(self, resources, *, connected=True):
        self._resource_request_in_flight = False
        scenes, sources = resources

        scene_values = scenes or (
            ["No scenes available"] if connected else ["OBS not connected"]
        )
        source_values = sources or (
            ["No sources available"] if connected else ["OBS not connected"]
        )
        self.comboboxes["scene_select"].configure(values=scene_values)
        self.comboboxes["source_select"].configure(values=source_values)
        self.comboboxes["scene_select"].set(scene_values[0])
        self.comboboxes["source_select"].set(source_values[0])

    def _obs_resources_error(self, error):
        self._resource_request_in_flight = False
        self._apply_obs_resources(([], []), connected=False)
        self.show_status(f"Could not load OBS resources: {error}", "#e05d5d")

    def load_scenes(self):
        """Compatibility alias for refreshing OBS resources."""
        self.load_obs_resources()
    ...
    def assign_scene(self):
        ...
        if scene in ["OBS not connected", "No scenes available", "Loading scenes..."]:
        ...
        self.show_status(
            f"Assigned scene '{scene}' to button {self.current_button}", "green"
        )
    ...
    def assign_source(self):
        ...
        if not self.current_button or self.current_button[0] != "P":
            self.show_status(
                "Please select a Volume potentiometer (P0-P3) first", "orange"
            )
            return
        ...
        # Update current assignment label
        self.labels["current_assignment"].configure(
            text=f"Current Assignment: {source}"
        )

        self.show_status(
            f"Assigned source '{source}' to potentiometer {self.current_button}",
            "green",
        )
    ...
    def assign_script(self):
        ...
        # Update current assignment label
        self.labels["current_assignment"].configure(
            text=f"Current Assignment: {script}"
        )

        self.show_status(
            f"Assigned script '{script}' to button {self.current_button}", "green"
        )
    ...
    def save_mappings(self):
        ...
        try:
            result = self.app.config.save_mapping()
        except Exception as exc:
            self.show_status(f"Could not save mappings: {exc}", "#e05d5d")
            return
        if result is False:
            self.show_status(
                "Could not save mappings. Check the application log.", "#e05d5d"
            )
        else:
            self.show_status("Mappings saved successfully", "#4caf6a")
